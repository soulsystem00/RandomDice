...
public class IronDice : MonoBehaviour, IAttack
{
    Dice dice;
    ...
    public IEnumerator Attack()
    {
        ...
        foreach (var attack in dice.AttackPos)
        {
            ...
            if(!dice.IsAIDice)
            {
                var target = EnemyList.i.Enemies.OrderByDescending(x => x.Health).FirstOrDefault();
                if (target.IsBoss)
                {
                    bullet.Init(dice.Damage, target, () => { target.GetDamage(dice.Damage); });
                }
                else
                {
            ...
            {
                var target = AIEnemyList.i.Enemies.OrderByDescending(x => x.Health).FirstOrDefault();
                if (target.IsBoss)
                {
                    bullet.Init(dice.Damage, target, () => { target.GetDamage(dice.Damage); });
                }
